...
public class ParquetReadWriteExample1 {
    ...
    private static void writeParquetFile(MessageType schema) throws IOException {
        ...
        // Create a ParquetWriter with the write support, output file path, and configuration
        String outputFile = "src/main/resources/example.parquet";
        Configuration configuration = new Configuration();
        try (ParquetWriter<Group> writer = new ParquetWriter<>(
                new org.apache.hadoop.fs.Path(outputFile),
    ...
    private static void readParquetFile(MessageType schema) throws IOException {
        ...
        String inputFile = "src/main/resources/example.parquet";
        Configuration configuration = new Configuration();
        try (ParquetReader<Group> reader = ParquetReader.builder(new GroupReadSupport(), new org.apache.hadoop.fs.Path(inputFile))
